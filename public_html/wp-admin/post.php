...
if ( isset( $_GET['post'] ) && isset( $_POST['post_ID'] ) && (int) $_GET['post'] !== (int) $_POST['post_ID'] ) {
	wp_die( __( 'A post ID mismatch has been detected.' ), __( 'Lo sentimos no esta autorizado a editar este item.' ), 400 );
...
if ( isset( $_POST['post_type'] ) && $post && $post_type !== $_POST['post_type'] ) {
	wp_die( __( 'Un post que no coincide fue detectado.' ), __( 'Lo sentimos no esta autorizado a editar este item.' ), 400 );
}
...
switch ( $action ) {
	case 'post-quickdraft-save':
		...
		if ( ! wp_verify_nonce( $nonce, 'add-post' ) ) {
			$error_msg = __( 'No se puede enviar este formulario, por favor refresque e intente nuevamente.' );
		}
	...
	case 'edit':
		...
		if ( ! $post ) {
			wp_die( __( 'Intestaste editar un item que ya no existe. Tal vez fue eliminado?' ) );
		}
	...
	case 'trash':
		...
		if ( ! $post ) {
			wp_die( __( 'El item que estas intentando mover a la papelera ya no existe.' ) );
		}

		if ( ! $post_type_object ) {
			wp_die( __( 'Posteo de tipo inválido.' ) );
		}

		if ( ! current_user_can( 'delete_post', $post_id ) ) {
			wp_die( __( 'Lo sentimos no se puede mover este item a la papelera' ) );
		}
		...
		if ( $user_id ) {
			...
			wp_die( sprintf( __( 'No puede mover este item a la papelera. %s esta siendo editado.' ), $user->display_name ) );
		}
